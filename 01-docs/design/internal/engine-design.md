...
| HC-006 休憩交代制（窓口維持） | 基本形は HC-001 × HC-003(d) の組み合わせで充足済み。厳格形（同時に複数名が休憩に入ることの一律禁止）は各（日, スロット）: `Σ_staff Σ_{開始が当該スロットを覆う} break_start ≤ 1` を設定 ON 時のみ付与（`options.strict_single_break`・初期 OFF）。P6-2 で実装 | 一部実装 |
