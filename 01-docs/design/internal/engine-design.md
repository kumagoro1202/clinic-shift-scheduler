...
### 5.1 P6-8 での確認結果（重みプリセット値は変更なし）

P6-8 でサンプルデータ（`config/samples/sample_schedule_202608.yaml`・
月次モデル）を3プリセットそれぞれで生成し、3.1 節で申し送られた「最小配置
トークン」（半日診療日に30分1コマのみの配置等）の発生頻度を観察した
（`03-tests/unit/test_p6_8_weight_preset_switch.py`）。

- 半日診療日における最小配置トークンの発生比率は、balance・skill_focus・
  days_focus のいずれも おおむね 20〜26% で、プリセット間に明確な悪化傾向は
  見られなかった（days_focus は SC-004/SC-005 の重みを引き上げるが、
  balance 比で顕著な悪化は観測されなかった）
- 発生自体は 3.1 節の一般教訓どおり SC-004+HC-007 の組み合わせによる構造的
  副作用であり、特定スキルにのみ資格を持つスタッフ（配置可能エリアが限られる
  スタッフ）に偏って発生する。P6-8 時点ではプリセット重みの追加調整・HC 追加は
  不要と判断した
- 上記は本節の重みプリセット値（初期値のまま）を確認するテストとして
  `TestMinimalPlacementTokenObservation` に固定化されている。将来プリセット値を
  再調整する場合は、同テストのしきい値（回帰ガード）を参照すること
