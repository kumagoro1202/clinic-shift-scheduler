...
データソースは scheduler.config_loader（診療所設定・ファイル直接編集のみ）と
scheduler.calendar / scheduler.engine（月次入力・生成エンジン）。対象月は
月次入力ファイル（schedule-YYYYMM.yaml）側で決まるため、本画面では
読み込んだファイルの対象月を表示し、最適化モードのみを画面から選択する
（P7-2/P7-3 と同様、1 ファイルを対象とする運用）。
"""
